...
void BookHistograms(ExRootResult *result, MyPlots *plots)
{
  ...
  plots->fPhotonPT[0] = result->AddHist1D(
    "photon_pt_0", "leading photon P_{T}",
    "photon P_{T}, GeV/c", "number of photons",
    100, 0.0, 100.0);

  plots->fPhotonPT[1] = result->AddHist1D(
    "photon_pt_1", "2nd leading photon P_{T}",
    "photon P_{T}, GeV/c", "number of photons",
    100, 0.0, 100.0);
  ...
  comment->AddText("demo-plot");
  comment->AddText("mono-higgs");
...
void read_photons(const char *inputFile)
{
